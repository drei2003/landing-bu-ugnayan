...
const GradualBlur: React.FC<GradualBlurProps> = props => {
  ...
  const blurDivs = useMemo(() => {
    ...
    const backgroundColor = resolvedTheme === 'dark' ? 'black' : 'white';
    ...
    for (let i = 1; i <= config.divCount; i++) {
      ...
      // Calculate opacity instead of blur
      let opacityValue: number;
      if (config.exponential) {
        opacityValue = Number(math.pow(progress, 2));
      } else {
      ...
      let gradient = `transparent ${p1}%, ${backgroundColor} ${p2}%`;
      if (p3 <= 100) gradient += `, ${backgroundColor} ${p3}%`;
      ...
      const divStyle: CSSProperties = {
        ...
        opacity: opacityValue * config.opacity,
        transition:
    ...
    return divs;
  }, [config, isHovered, mounted, resolvedTheme]);
